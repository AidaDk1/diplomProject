...
def collections2(request):
    ...
    return render(request,'collections2.html',context)

def collections3(request):
    pr=models.Product.objects.all()
    context={'Product':pr}
    return render(request,'collections3.html',context)

def subcategoryview(request,title):
    if(Category.objects.filter(title=title)):
        sub=SubCategory.objects.filter(category__title=title)
        sub_name=SubCategory.objects.filter(title=title).first()
        context={'SubCategory':sub,'subcategory_name':sub_name}
        return render(request,'subcategory.html',context)


def productsview(request,id):
    if (SubCategory.objects.filter(id=id)):
        pr= Product.objects.filter(sub_category_id=id)
        pro_name = Product.objects.filter(id=id).first()
        context = {'Product': pr, 'product_name': pro_name}
...
def admin_category_view(request):
    ...
    return render(request, 'admin_categories.html', {'category': category})
...
def admin_add_category_view(request):
    ...
    return render(request, 'admin_add_categories.html', {'categoryForm': categoryForm})

# @login_required(login_url='adminlogin')
# def admin_add_category_view(request):
#     if request.method == 'POST':
#         title1=request.POST['title']
#         img2 = request.POST['img']
#         category= Category()
...
def admin_add_subcategory_view(request):
    subcategoryForm = forms.SubCategoryForm()
    if request.method == 'POST':
        subcategoryForm = forms.SubCategoryForm(request.POST, request.FILES)
        if subcategoryForm.is_valid():
            subcategoryForm.save()
        return HttpResponseRedirect('admin-products')
    return render(request, 'admin_add_subcategories.html', {'subcategoryForm': subcategoryForm})
...
#         img2 = request.POST.get('img')
#         subcategory= SubCategory()
#         subcategory.category=cat
#         subcategory.title = title1
#         subcategory.photo = img2
#         subcategory.save()
#     category = Category.objects.all()
#     return render(request, 'admin_add_subcategories.html',context={'category':category})
...
def delete_subcategory_view(request, pk):
    subcategory = models.SubCategory.objects.get(id=pk)
    subcategory.delete()
    return redirect('admin-products')
...
def update_category_view(request, pk):
    ...
    if request.method == 'POST':
        categoryForm = forms.CategoryForm(request.POST, request.FILES, instance=category)
        if categoryForm.is_valid():
            categoryForm.save()
            return redirect('admin-products')
    return render(request, 'admin_update_category.html', {'categoryForm': categoryForm})
...
#ABOUT US CONTACT US PAGE VIEWS
